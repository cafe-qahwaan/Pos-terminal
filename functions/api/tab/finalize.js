...
export async function onRequestPost({ request, env }) {
  ...
  if (!tab_id || !payment_method || !staff) return new Response("Missing tab_id, payment_method, or staff", { status: 400 });

  const tabUrl = `${env.SUPABASE_URL}/rest/v1/tabs?id=eq.${encodeURIComponent(tab_id)}&select=id,spot,status,cart&limit=1`;
  const tabRes = await fetch(tabUrl, { headers:{ apikey: env.SUPABASE_SERVICE_KEY, Authorization:`Bearer ${env.SUPABASE_SERVICE_KEY}`, Accept:"application/json" }});
  ...
  const orderRow = {
    ...
    amount_received: received, change_due
  };
...
/* helpers */
function toArray(v){ if(Array.isArray(v)) return v; if(v==null) return []; try{const p=typeof v==="string"?JSON.parse(v):v; return Array.isArray(p)?p:[]}catch{return[]} }
function money(n){ return "£"+Number(n||0).toFixed(2); }
function esc(s){ return String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"); }

/* --- receipt with Print + New Order buttons (hidden on print) --- */
...
<style>
  ...
  .right{float:right}
  .toolbar{position:sticky;top:0;background:#fff;padding:10px 0;border-bottom:1px solid #ddd;margin-bottom:12px}
  .toolbar button{padding:10px 14px;border:none;border-radius:8px;margin-right:8px;cursor:pointer}
  .print{background:#1f8fff;color:#fff}
...
<body>
  ...
  <p><strong>Payment:</strong> ${esc(order.payment_method)} &nbsp; 
     <strong>Received:</strong> ${money(order.amount_received)} &nbsp; 
     <strong>Change:</strong> ${money(order.change_due)}</p>
