...
export async function onRequestPost({ request, env }) {
  ...
  // Insert order (NO created_at field)
  const orderRow = {
    cart,
    subtotal,
    payment_method,
  ...
  // Return receipt
  return new Response(renderReceipt("Cafe Qahwaan — Receipt", savedOrder), {
...
function renderReceipt(title, order){
  const lines=(order.cart||[]).map(i=>`<tr><td>${esc(i.name)}</td><td style="text-align:center">${i.qty}</td><td style="text-align:right">${money(i.price*i.qty)}</td></tr>`).join("");
  return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title>
  <style>body{font-family:system-ui,Arial,sans-serif;padding:20px;color:#222}
  h1{font-size:18px;margin:0 0 8px}table{width:100%;border-collapse:collapse;margin-top:10px}
  td{padding:4px 0;border-bottom:1px dashed #ccc}.right{float:right}</style></head>
  <body><h1>${esc(title)}</h1>
  <div>Spot: <strong>${esc(order.spot||"-")}</strong></div>
  <table>${lines}</table>
  ...
  <hr><p>Thanks for visiting Cafe Qahwaan!</p></body></html>`;
}
